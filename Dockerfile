...
RUN cd /&&\
    git clone https://github.com/Nemirtingas/osxcross_vcpkg --branch=SDK${OSX_VER}_clang${CLANG_VER}_target /osxcross_tmp &&\
    tar xzf /osxcross_tmp/osxcross_${OSX_VER}_clang${CLANG_VER}.tgz &&\
    rm -rf /osxcross_tmp &&\
    git clone --depth=1 https://github.com/tpoechtrager/osxcross.git /osxcross_b &&\
